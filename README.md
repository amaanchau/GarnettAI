...
- **Domain-restricted web search** — the agent can call a web search tool powered by GPT-5.4-nano with OpenAI's `web_search` API, restricted to trusted domains (TAMU catalog, Reddit, RateMyProfessors, Coursicle, etc.) via `allowed_domains` filtering
- **Web search source pills** — when web search is used, source references appear as compact animated pills below the AI response with favicons and domain names, linking to the original pages
- **Streaming responses** — real-time SSE streaming with tool call progress chips (spinner while running, checkmark on completion)
- **Course and professor link pills** — clickable pills after each AI response link directly to the Anex detail pages for both courses and professors
...
- **Course detail view**
  ...
  - Instructor names are clickable links to their professor overview page
...
- **Cross-linking** — navigate between course and professor views seamlessly via reactive URL params (`?course=` and `?professor=`)
- **Optimized professor queries** — the `professor` table includes a `course_tables` column indexing which course tables each professor appears in, reducing professor lookups from ~10s to <1s
...
- Framer Motion animations throughout (source pills, course chips, modals, page transitions)
...
| Web Search | OpenAI GPT-5.4-nano + `web_search` with domain filtering |
...
- Node.js 18+
- A PostgreSQL database with TAMU grade distribution tables (one table per course, e.g. `csce121`, `math151`) and a `professor` table with a `course_tables text[]` column
- An OpenAI API key
...
│   │   ├── CourseDataTable.tsx   # Grade distribution table (instructor names link to professor pages)
│   │   ├── CourseLinkCard.tsx    # Course + professor link pills
│   │   └── MarkdownMessage.tsx   # AI response markdown renderer
│   └── lib/rag/
│       ├── run-agent.ts          # Agent orchestration (streamText + web source capture)
│       ├── rag-tools.ts          # Tool definitions (GPA, RMP, web search with domain filtering)
│       ├── prompts.ts            # System prompt and message building
│       ├── prefetch.ts           # Pre-fetch pipeline for selected courses
│       ├── queries.ts            # Database query functions (uses course_tables for fast professor lookups)
